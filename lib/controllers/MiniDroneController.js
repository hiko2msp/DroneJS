...
var MiniDroneController = exports.MiniDroneController = function (_EventEmitter) {
    ...
    _createClass(MiniDroneController, [{
            ...
            return new Promise(function (resolve, reject) {
                _this2.droneService.startScanning().then(function () {
                    _this2.droneService.discoverDevice(droneIdentifier).then(function (peripheral) {
                        _this2.device = peripheral.advertisement.localName;
        ...
        value: function sendPilotingCommand(categoryName, cmdName, args) {
            ...
            return new Promise(function (resolve, reject) {
                args = args || [];
                var addLength = cmdName === 'maneuver' || cmdName === 'clawControl' ? args.length + 3 : args.length;
        ...
        value: function addListeners() {
            ...
            this.cmdSubscription = this.droneService.cmdObservable.distinctUntilChanged().subscribe(function (data) {
                ...
                if (os.platform() !== 'win32') {
                    ...
                    if (navInfo.name === 'pictureStateChangedV2') {
                        var navObj = {};
                        navInfo.args.forEach(function (arg) {
                            navObj[arg.name] = arg.value;
                        });
            ...
            var ignoreBytes = 0;
            this.ftpSubscription = this.droneService.ftpObservable.distinctUntilChanged(function (prev, curr) {
                var str = curr.toString('utf8');
                if (_this7.device.indexOf('RS') < 0 && str.toLowerCase().indexOf('delete successful') < 0) {
        ...
        value: function parseData(data) {
            ...
            const projectId = data.readUInt8(offset);
            cmds = projectId === this.droneCmds.project.id ? this.droneCmds : this.commonCmds;
        ...
        value: function getValuesByType(buffer, offset, args) {
            ...
            args.forEach(function (arg) {
                ...
                switch (arg.type) {
                    ...
                    case 'enum':
                        var id = buffer.readUInt32LE(offset);
