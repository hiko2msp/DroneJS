...
var MiniDroneView = exports.MiniDroneView = function () {
    ...
    _createClass(MiniDroneView, [{
        ...
        value: function connect(droneIdentifier) {
            ...
            return new Promise(function (resolve, reject) {
                _this.droneController.connect(droneIdentifier).then(function (peripheral) {
                    if (!peripheral) {
                        reject('Drone not connected successfully');
                    }

                    _this.device = peripheral;
        ...
        key: 'grabOpen',
        value: function grabOpen() {
            var _this30 = this;

            return new Promise(function (resolve, reject) {
                _this30.droneController.sendPilotingCommand('usbAccessory', 'clawControl', [0, 0]).then(function () {
                  if (os.platform() === 'win32') {
                    (0, _sleep.sleep)(1000).then(function () {
                      resolve('success');
                    });
                  } else {
                    var callback = function callback(navObj) {
                      if (navObj.state === 'opened') {
                        (0, _debug.debug)('Drone has closed grab');
                        _this30.droneController.removeListener('clawState', callback);
                        (0, _sleep.sleep)(_this30.breathingTime).then(function () {
                          resolve('success');
                        });
                      } else if (navObj.state === 'opening') {
                        (0, _debug.debug)('sending flat trim');
                        _this30.flatTrim();
                      }
                    };
                    _this30.droneController.on('clawState', callback);
                  }
                }).catch(function (e) {
                    reject(e);
                });
        ...
        key: 'grabClose',
        value: function grabClose() {
            var _this31 = this;

            return new Promise(function (resolve, reject) {
                _this31.droneController.sendPilotingCommand('usbAccessory', 'clawControl', [0, 1]).then(function () {
					if (os.platform() === 'win32') {
						(0, _sleep.sleep)(1000).then(function () {
							resolve('success');
						});
						...
						var callback = function callback(navObj) {
							if (navObj.state === 'closed') {
								(0, _debug.debug)('Drone has closed grab');
								_this31.droneController.removeListener('clawState', callback);
								(0, _sleep.sleep)(_this31.breathingTime).then(function () {
									resolve('success');
								});
							} else if (navObj.state === 'closing') {
								(0, _debug.debug)('sending flat trim');
								_this31.flatTrim();
							}
						};
						_this31.droneController.on('clawState', callback);
					}
                }).catch(function (e) {
                    reject(e);
                });
        ...
        value: function takeOff() {
            ...
            return new Promise(function (resolve, reject) {
                _this4.droneController.sendPilotingCommand('piloting', 'takeOff').then(function () {
                  if (os.platform() === 'win32') {
                    (0, _sleep.sleep)(1500).then(function () {
                      resolve('success');
                    });
                  } else {
                    var callback = function callback(navObj) {
        ...
        value: function takePicture() {
            ...
            return new Promise(function (resolve, reject) {
                var cmd = _this17.device.indexOf('RS') > -1 ? 'picture' : 'pictureV2',
                    args = cmd === 'picture' ? [0] : null;

                _this17.droneController.sendPilotingCommand('mediaRecord', cmd, args).then(function () {
                    var callback = function callback(navObj) {
                        if (navObj.state === 'ready') {
                            (0, _debug.debug)('Drone can take a picture');
                            _this17.droneController.removeListener('pictureStateChangedV2', callback);
                            resolve('success');
                        } else if (navObj.state === 'busy') {
                            (0, _debug.debug)('Drone cannot take a picture');
                            _this17.flatTrim();
                        }
                    };
                    _this17.droneController.on('pictureStateChangedV2', callback);
                }).catch(function (e) {
                    reject(e);
                });
            });
                ...
                var callback = function callback(mediaObj) {
                    if (mediaObj.state === 'GET' || mediaObj.state === 'MD5 OK') {
                        ...
                        if (dataStr.toLowerCase().indexOf('end of transfer') >= 0) {
                            ...
                            if (_this19.dirName.indexOf('Spider') < 0) {
                                _this19.flatTrim();
                            }
                        } else {
        ...
        value: function downloadThumbnail(name, downloadPath) {
            var _this32 = this;

            return new Promise(function (resolve, reject) {
                var stream = _this32.fs.createWriteStream(downloadPath + '/' + name);
                ...
                var callback = function callback(mediaObj) {
                    if (mediaObj.state === 'GET' || mediaObj.state === 'MD5 OK') {
                        ...
                        if (dataStr.toLowerCase().indexOf('end of transfer') >= 0) {
                            (0, _debug.debug)('Closing the stream');
                            stream.end();
                            _this32.droneController.removeListener('media-data', callback);
                            (0, _sleep.sleep)(_this32.breathingTime).then(function () {
                                resolve('success');
                            });
                        } else if (dataStr.indexOf('MD5') >= 0) {
                            (0, _debug.debug)('Packet ended');
                            _this32.droneController.sendMediaCommand('MD5 OK', '').then(function () {}).catch(function (e) {
                                reject(e);
                            });

                            if (_this32.dirName.indexOf('Spider') < 0) {
                                _this32.flatTrim();
                            }
                        } else {
                            stream.write(mediaObj.data.slice(1));
                ...
                if (!_this32.dirName) {
                    _this32._mediaDir().then(function (dir) {
                        _this32.dirName = dir;
                        _this32.droneController.on('media-data', callback);
                        _this32.droneController.sendMediaCommand('GET', '/internal_000/' + _this32.dirName + '/thumb/' + name).then(function () {}).catch(function (e) {
                            reject(e);
                        });
                    }).catch(function (e) {
                        reject(e);
                ...
                var callback = function callback(mediaObj) {
                    if (mediaObj.state === 'LIS') {
                        ...
                        if (str.toLowerCase().indexOf('end of transfer') >= 0) {
                            ...
                            if (_this25.device.indexOf('RS') > -1) {
                                (0, _sleep.sleep)(_this25.breathingTime).then(function () {
                                    resolve(folderArr[0]);
                                });
                            } else {
                                _this25.flatTrim().then(function () {
                                    resolve(folderArr[0]);
                                });
                            }
                        }
